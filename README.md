...
9. イベント検知からクリップ実行までの遅延秒数も設定画面で変更できます。
...
## `ffmpeg_record.sh`（macOS）

リプレイバッファではなく、録画開始から終了までをそのまま MP4 として保存する簡易スクリプトです。`avfoundation` 入力を利用して画面をキャプチャし、Esc もしくは **q** を押すと終了します。

使用例:

```bash
./ffmpeg_record.sh -f 30 -b 20M -s "1:none" -o ~/Movies
```
...
- `-b BITRATE` – 目標ビットレート（デフォルト `20M`）。
- `-s SRC` – avfoundation 用のキャプチャソース（デフォルト `"1:none"`）。
- `-o OUT_DIR` – 保存先ディレクトリ（デフォルト `~/Movies`）。
